 ...
 */
public class shici_title_GridviewAdapter extends BaseAdapter {
    private int selectedPosition;
    private List<shici_title_gridview_bean> mlist;//用于保存传递进来的   数据
    //数据源与适配器关联
    private LayoutInflater mInflater;
    public shici_title_GridviewAdapter(Context context, List<shici_title_gridview_bean> list) {
            mlist=list;
   ...
   //这句是把listview的点击position,传递过来
    ...
    public View getView(int position, View convertView, ViewGroup parent) {//返回每一项的显示内容
        ...
        shici_title_gridview_bean bean = mlist.get(position);
        if(bean.num ==3 ){
            v = mInflater.inflate(R.layout.shici_title_gridview_item, null);
        }
