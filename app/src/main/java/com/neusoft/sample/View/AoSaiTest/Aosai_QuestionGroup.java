...
import com.neusoft.sample.Model.PostNoStudentsChampionship;
 ...
 */
public class Aosai_QuestionGroup extends Aosai_Subject {
    ...
    public void notifyObservers() {
        for (Aosai_Observer o : observers) {
            o.update(this);
        }
    }
        ...
        if (!"doagain".equals(MsharedPrefrence.GetisDoGain(ContextHolder.getContext()))) {
            ...
            if ((int) num_right != questions.size()) {//不满分
                ...
                } else //不是学生
                {
                    ...
                    String zuNo = questions.get(1).getItemNo().substring(0, 11);
                    String JiaocaiSBNo = questions.get(1).getItemNo().substring(0, 5);
                    hmap.put("zuNo", zuNo);
