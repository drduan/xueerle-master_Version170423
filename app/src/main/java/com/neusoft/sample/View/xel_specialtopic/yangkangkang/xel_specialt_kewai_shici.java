...
public class xel_specialt_kewai_shici extends AppCompatActivity {
    Typeface tf;
    Typeface tttf;
    private TextView tv_stu_autor,tv_stu_shang,tv_title;
    private AlwaysMarqueeTextView tv_shici_Navigation;
    private GridView lv_stu_shici;
    private ImageView im_voice,img_shangxi;
    private LinearLayout lin_jiexis;
    ...
    private void Inti_add() {
        ...
        tv_shici_Navigation.setText("专题>语文>拓展古诗词>第"+hou_4+"章>第"+hou_2+"首");
    }
    //////添加向  listview   中添加东西
    private void Ini_add_list() {
        ...
        for(int i = 0;i<accept.size();i++){
            ...
            if(i ==0)
            {
                ...
                shangxi.replaceAll(" ","");
                if(shangxi.length()<4){
                    lin_jiexis.setVisibility(View.GONE);
                }
            }else {
            }
            if(int_.length() ==10){
    ...
    private void Initview() {
        ...
        lin_jiexis = (LinearLayout) findViewById(R.id.lin_jiexi);
...
//            
    ...
    private void Ini_bofang() {
        im_voice.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(xel_specialt_kewai_shici.this, "正在加载音频请等待。。。。。。", Toast.LENGTH_LONG).show();
