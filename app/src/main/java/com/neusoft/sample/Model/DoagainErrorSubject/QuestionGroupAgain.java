 ...
 */
public class QuestionGroupAgain extends SubjectDoagain {
    ...
    public void notifyObservers() {
        for (ObserverDoagain o : observers) {
            o.update(this);
        }
    }
    ...
    private void report() {
        ...
        for (Question question : questions) {
            if (question.judege()) {
                num_right++;
                //查询db中的东西
                ErrorSubjectNub errorSubjectNub= App.getDaoSession(ContextHolder.getContext()).queryBuilder(ErrorSubjectNub.class).where(ErrorSubjectNubDao.Properties.Examination_number.like(question.getItem_no())).unique();
...
//                        String CSJG = JSON.toJSONString(xtcsjg);
                        ...
                        Log.i("@@@", f + "" + num_right);
                        XTCSJG xtcsjg = new XTCSJG();
                        String id = questions.get(1).getItem_no().substring(0, 11);
                        xtcsjg.setTest_group_number(id);
                        xtcsjg.setScore((int) (num_right * f));
