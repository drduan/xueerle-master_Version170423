...
/**
 * Created by xudong on 2017/7/16.
 */

public class BizParam {


    /** 128	对一笔交易的具体描述信息。如果是多种商品，请将商品描述字符串累加传给body。	Iphone6 16G*/
    private String body	;
    /** 	256	商品的标题/交易标题/订单标题/订单关键字等。	大乐透*/
    private String subject;
    private String out_trade_no;//		是	64	商户网站唯一订单号	70501111111S001111119
    private String timeout_express	;	//否	6	设置未付款支付宝交易的超时时间，一旦超时，该笔交易就会自动被关闭。当用户进入支付宝收银台页面（不包括登录页面），会触发即刻创建支付宝交易，此时开始计时。取值范围：1m～15d。m-分钟，h-小时，d-天，1c-当天（1c-当天的情况下，无论交易何时创建，都在0点关闭）。 该参数数值不接受小数点， 如 1.5h，可转换为 90m。	90m
    private String total_amount	;//	是	9	订单总金额，单位为元，精确到小数点后两位，取值范围[0.01,100000000]	9.00
    private  String seller_id;//		否	16	收款支付宝用户ID。 如果该值为空，则默认为商户签约账号对应的支付宝用户ID	2088102147948060
    private String product_code;//		是	64	销售产品码，商家和支付宝签约的产品码，为固定值QUICK_MSECURITY_PAY	QUICK_MSECURITY_PAY
    private String goods_type;		//否	2	商品主类型：0—虚拟类商品，1—实物类商品
    //注：虚拟类商品不支持使用花呗渠道	0
...
//    注：与enable_pay_channels互斥	pcredit,moneyFund,debitCardExpress
    ...
    public String getNable_pay_channels() {
        return nable_pay_channels;
    }

    public void setNable_pay_channels(String nable_pay_channels) {
        this.nable_pay_channels = nable_pay_channels;
    }
